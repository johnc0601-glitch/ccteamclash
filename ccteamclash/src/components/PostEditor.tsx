...
export function PostEditor(){
  ...
  useEffect(()=>{const saved=localStorage.getItem('cc-draft');if(saved){try{setPost(JSON.parse(saved))}catch{}}},[]);
  useEffect(()=>{localStorage.setItem('cc-draft',JSON.stringify(post));setStatus('Draft saved automatically')},[post]);
  const slug=useMemo(()=>post.title.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),[post.title]);
  function set(k:keyof Draft,v:string){setPost(p=>({...p,[k]:v}))}
  function useImage(file?:File){if(!file)return;set('imageName',file.name);setPreview(URL.createObjectURL(file))}
  function photo(e:React.ChangeEvent<HTMLInputElement>){useImage(e.target.files?.[0])}
  ...
  return <div className="editor-workspace" onPaste={e=>{const image=[...e.clipboardData.items].find(i=>i.type.startsWith('image/'))?.getAsFile();if(image)useImage(image)}}>
    ...
    <div className="editor-layout">
      <form className="real-editor" onSubmit={e=>e.preventDefault()}>
        ...
        <div className="editor-actions"><button className="publish-action" onClick={download}>Save publish file</button><button className="secondary" onClick={()=>{setPost(blank);setPreview('');localStorage.removeItem('cc-draft')}}>Start over</button></div>
      </form>
