...
export async function unlockCaptainRoster(formData: FormData) {
  ...
  const {profile} = await requireCommissioner(matchId);
  const admin = createAdminClient() as any;
  const {data: match} = await admin.from('launch_schedule_matches').select('home_team_id,away_team_id,status').eq('id', matchId).maybeSingle();
  ...
  const {data: existing} = await admin.from('launch_match_roster_unlocks').select('id').eq('match_id', matchId).eq('team_id', teamId).is('relocked_at', null).maybeSingle();
  if (!existing) {
    const {error} = await admin.from('launch_match_roster_unlocks').insert({match_id: matchId, team_id: teamId, unlocked_by: profile.id});
...
export async function cancelCaptainRosterUnlock(formData: FormData) {
  ...
  const {profile} = await requireCommissioner(matchId);
  const admin = createAdminClient() as any;
  await admin.from('launch_match_roster_unlocks').update({relocked_at: new Date().toISOString(), relocked_by: profile.id}).eq('match_id', matchId).eq('team_id', teamId).is('relocked_at', null);
  revalidatePath(`/matches/${matchId}`);
...
async function requireCommissioner(matchId: string) {
  ...
  const admin = createAdminClient() as any;
  const {data: profile} = await admin.from('launch_profiles').select('id,role,status').eq('user_id', user.id).maybeSingle();
