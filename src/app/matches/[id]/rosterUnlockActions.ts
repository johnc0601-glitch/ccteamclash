...
export async function unlockCaptainRoster(formData: FormData) {
  ...
  const {supabase, profile, matchHref} = await requireCommissioner(matchId);
  ...
  if (!match || !CORRECTABLE_STATUSES.has(match.status) || ![match.home_team_id, match.away_team_id].includes(teamId)) {
    redirect(`${matchHref}?commissionerError=${encodeURIComponent('That roster cannot be unlocked.')}`);
  }
  ...
  if (!existing) {
    ...
    if (error) redirect(`${matchHref}?commissionerError=${encodeURIComponent('Roster unlock failed.')}`);
  }
  revalidatePath(`/matches/${matchId}`);
  revalidatePath(matchHref);
  redirect(`${matchHref}?commissionerNotice=${encodeURIComponent('Roster unlocked for the captain. It will lock again when the captain saves the correction.')}`);
}
...
export async function cancelCaptainRosterUnlock(formData: FormData) {
  ...
  if (error) redirect(`${matchHref}?commissionerError=${encodeURIComponent('Roster unlock could not be cancelled.')}`);
  revalidatePath(`/matches/${matchId}`);
  revalidatePath(matchHref);
  redirect(`${matchHref}?commissionerNotice=${encodeURIComponent('Roster unlock cancelled.')}`);
}
...
async function requireCommissioner(matchId: string) {
  ...
  if (!profile || profile.role !== 'Commissioner' || profile.status !== 'Approved') redirect(`${matchHref}?commissionerError=${encodeURIComponent('Commissioner access is required.')}`);
  return {supabase, profile, matchHref};
}
