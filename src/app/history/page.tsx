...
function ExploreRecords({seasonName, seasonId, archiveUrl}: {seasonName?: string; seasonId?: string; archiveUrl?: string}) {
  ...
  return <section className={styles.archiveLinks}><div><strong>{seasonName ? `Explore ${seasonName}` : 'Explore the records'}</strong><span>History preserves league outcomes. Stats holds the detailed player numbers; Rankings holds current season order and Clash Index.</span></div><div className={styles.linkActions}><Link href={statsHref}>Stats →</Link><Link href="/rankings">Rankings →</Link><Link href="/players">Player records →</Link><Link href="/teams">Team history →</Link>{archiveUrl ? <a href={archiveUrl} target="_blank" rel="noreferrer">Full match archive →</a> : null}</div></section>;
}
