...
type MainTab = 'season' | 'stats' | 'clash';
type Division = 'open' | 'women' | 'junior';
type RankingsClientProps = {
  current?: SeasonRankingGroup;
  overall: SeasonRankingGroup;
  history: SeasonRankingGroup[];
  clash: {open: ClashRankingEntry[]; women: ClashRankingEntry[]; junior: ClashRankingEntry[]};
  teamColors: Record<string, string>;
  initialView?: MainTab;
  initialSeasonId?: string;
};

export function RankingsClient({current, overall, history, clash, teamColors, initialView = 'season', initialSeasonId}: RankingsClientProps) {
  const [tab, setTab] = useState<MainTab>(initialView);
  ...
  return <>
    ...
    {tab === 'season' ? (current ? <RankingSection group={current} eyebrow="Current season" includeJunior onOpen={setSelectedEntry} teamColors={teamColors} /> : <p className={styles.emptyState}>No current-season rankings are available yet.</p>) : null}
    {tab === 'stats' ? <StatsSection overall={overall} groups={history} initialSeasonId={initialSeasonId} onOpen={setSelectedEntry} teamColors={teamColors} /> : null}
...
function RankingSection({group, eyebrow, includeJunior, onOpen, teamColors}: {group: SeasonRankingGroup; eyebrow: string; includeJunior: boolean; onOpen: (entry: HistoricalRankingEntry) => void; teamColors: Record<string, string>}) {
  const [division, setDivision] = useState<Division>('open');
  const [showAll, setShowAll] = useState(false);
  const entries = division === 'women' ? group.women : division === 'junior' ? group.junior ?? [] : group.open;
  const visible = showAll ? entries : entries.slice(0, 5);
  return <section className={styles.tabPanel}><header className={styles.sectionHeading}><div><span className="eyebrow">{eyebrow}</span><h2>{group.seasonName}</h2></div></header><DivisionTabs division={division} onChange={(next) => {setDivision(next); setShowAll(false);}} includeJunior={includeJunior} /><SeasonTable entries={visible} onOpen={onOpen} teamColors={teamColors} />{entries.length > 5 ? <button type="button" className={styles.viewAll} onClick={() => setShowAll((value) => !value)}>{showAll ? 'Show Top 5' : `View All ${entries.length}`}</button> : null}</section>;
}

function StatsSection({overall, groups, initialSeasonId, onOpen, teamColors}: {overall: SeasonRankingGroup; groups: SeasonRankingGroup[]; initialSeasonId?: string; onOpen: (entry: HistoricalRankingEntry) => void; teamColors: Record<string, string>}) {
  const validInitial = initialSeasonId && groups.some((group) => group.seasonId === initialSeasonId) ? initialSeasonId : 'overall';
  ...
  return <section>
    ...
    <RankingSection group={selected} eyebrow={seasonId === 'overall' ? 'Career history' : 'Season history'} includeJunior={false} onOpen={onOpen} teamColors={teamColors} />
  </section>;
}
