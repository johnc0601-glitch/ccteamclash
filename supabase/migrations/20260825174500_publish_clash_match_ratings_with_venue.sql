-- Preserve the frozen CI venue through the atomic Matchday publication boundary.
-- The venue comes from the pre-match snapshot (course ownership at capture time),
-- not from a later lookup of the current team/course profile.
...
begin
  ...
  select
    x.contest_id, x.player_id, x.team_id, x.side, x.venue, x.format, x.outcome,
  ...
  from jsonb_to_recordset(p_facts) as x(
    contest_id text, player_id text, team_id text, side text, venue text, format text, outcome text,
  ...
  if exists (select 1 from pg_temp.clash_publish_facts where venue not in ('Home', 'Neutral') or venue is null) then
    raise exception 'Rating facts contain an invalid or missing CI venue.' using errcode = '23514';
  end if;
  ...
  if (select count(distinct venue) from public.clash_match_rating_snapshots where match_id = p_match_id) <> 1 then
    raise exception 'Frozen Matchday CI snapshot contains mixed venue classifications.' using errcode = '23514';
  end if;
  ...
  insert into public.clash_contest_rating_facts (
    contest_id, match_id, player_id, team_id, player_name, team_name, side, venue, format,
  ...
  select
    f.contest_id, p_match_id, f.player_id, f.team_id, p.name, t.name, f.side, f.venue, f.format,
...
comment on function private.publish_clash_match_ratings(text, text, jsonb) is
  'Atomically validates published Matchday CI facts against frozen ratings and venue, persists immutable facts, updates each player once, preserves ghost provenance, and records idempotent publication.';
