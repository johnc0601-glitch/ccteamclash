...
begin
  ...
  -- Serialize publication for this Matchday and require official results first.
  ...
  if exists (
    select 1 from pg_temp.clash_publish_facts
    group by contest_id, player_id having count(*) <> 1
  ) then
    raise exception 'Duplicate contest/player rating fact.' using errcode = '23505';
  end if;

  -- Every submitted fact must exactly correspond to the persisted official result.
  ...
  left join public.launch_result_contest_players cp
    on cp.contest_id = f.contest_id and cp.player_id = f.player_id
  where c.id is null
     or cp.player_id is null
     or cp.team_id <> f.team_id
     or cp.side <> f.side
     or c.format <> f.format
     or (case when f.side = 'Home' then c.home_outcome else c.away_outcome end) <> f.outcome
     or f.win_probability not between 0 and 1
     or f.expected_points not between 0 and 1
  ...
  -- Require one fact for every persisted contest participant: no partial CI publication.
  select count(*) into v_bad_count
  from public.launch_result_contests c
  join public.launch_result_contest_players cp on cp.contest_id = c.id
  left join pg_temp.clash_publish_facts f
    on f.contest_id = c.id and f.player_id = cp.player_id
  ...
  if exists (
    ...
    left join public.clash_match_rating_snapshots s
      on s.match_id = p_match_id and s.player_id = f.player_id
    where s.player_id is null
       or s.algorithm_version <> p_algorithm_version
       or s.clash_index_before <> f.clash_index_before
       or s.team_id <> f.team_id
       or s.side <> f.side
  ) then
  ...
  -- Lock every affected player. Re-check current CI after the lock: another Matchday
  -- cannot silently move a player between snapshot and publication.
  perform 1
  ...
  -- Persist immutable contest facts while names are copied from canonical rows.
  insert into public.clash_contest_rating_facts (
    ...
    actual_points, expected_points, performance_vs_expected, ci_delta,
  ...
  select
    ...
    f.actual_points, f.expected_points, f.performance_vs_expected, f.ci_delta,
    p_algorithm_version, v_now
  from pg_temp.clash_publish_facts f
  join public.launch_players p on p.id = f.player_id
  join public.launch_teams t on t.id = f.team_id;

  -- The only live CI write: one aggregate update per player, from the same frozen CI.
  ...
  insert into public.clash_match_publications (
    ...
    p_match_id, p_algorithm_version, v_snapshot_count, v_fact_count, v_player_count, v_now
  );
...
comment on function private.publish_clash_match_ratings(text, text, jsonb) is
  'Atomically validates a complete published Matchday CI plan, persists immutable facts, updates each player once, and records idempotent publication.';
