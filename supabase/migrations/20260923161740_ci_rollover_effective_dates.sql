alter table public.launch_players add column pdga_rating_effective_date date;
comment on column public.launch_players.pdga_rating_effective_date is
  'PDGA published effective date for the stored rating; never the lookup or sync date.';

alter table public.historical_team_matches add column played_on date;
comment on column public.historical_team_matches.played_on is
  'Verified match date when known. NULL retains month-only archive precision.';

create table public.clash_season_rollovers (
  season_id text not null references public.launch_seasons(id),
  player_id text not null references public.launch_players(id),
  prior_season_id text references public.launch_seasons(id),
  prior_final_ci integer,
  prior_final_match_date date,
  match_date_basis text not null check (match_date_basis in ('Exact', 'MonthUpperBound', 'Unknown')),
  pdga_rating integer,
  pdga_rating_effective_date date,
  starting_ci integer not null,
  rule text not null check (rule in ('RecentPdga50', 'Pdga20', 'CarryForward', 'NewPdga', 'Provisional')),
  rule_version text not null default '2026-09-recent-pdga-v1',
  applied_at timestamptz not null default now(),
  applied_by uuid,
  primary key (season_id, player_id)
);
alter table public.clash_season_rollovers enable row level security;
grant select on public.clash_season_rollovers to authenticated;
create policy "commissioners read CI rollovers" on public.clash_season_rollovers
  for select to authenticated using (private.is_launch_commissioner());

-- Clear stale provenance when another writer changes rating/identity without a date.
-- The JSON guard protects this new field even in older installations whose player
-- guards enumerate protected fields instead of comparing the complete record.
create function private.guard_pdga_effective_date() returns trigger
language plpgsql set search_path = '' as $$
begin
  if new.pdga_rating_effective_date is distinct from old.pdga_rating_effective_date
     and current_user not in ('postgres','service_role','supabase_admin')
     and not private.is_launch_commissioner() then
    raise exception 'Only commissioners may change PDGA rating dates.' using errcode = '42501';
  end if;
  if new.pdga_rating is null
     or (new.pdga_number is distinct from old.pdga_number
         and new.pdga_rating_effective_date is not distinct from old.pdga_rating_effective_date)
     or (new.pdga_rating is distinct from old.pdga_rating
         and new.pdga_rating_effective_date is not distinct from old.pdga_rating_effective_date) then
    new.pdga_rating_effective_date := null;
  end if;
  return new;
end;
$$;
create trigger guard_pdga_effective_date before update on public.launch_players
for each row execute function private.guard_pdga_effective_date();

create function private.clash_season_start_ci(prior_ci integer, pdga integer, effective_on date, last_match date, division text)
returns integer language sql immutable set search_path = '' as $$
  select case
    when prior_ci is null then coalesce(pdga, case when division = 'Women' then 700 else 825 end)
    when pdga is null then prior_ci
    when effective_on > last_match then round((prior_ci + pdga)::numeric / 2)::integer
    else round(prior_ci::numeric * 0.8 + pdga::numeric * 0.2)::integer
  end;
$$;

-- Administrative boundary: derive from frozen prior-season truth, not current CI.
-- Snapshot reads require elevated access because those tables intentionally have
-- no public policies. The caller is checked before any read or write.
create function private.apply_clash_season_rollover(target_season_id text, target_player_id text, preview_only boolean)
...
$$;
revoke all on function private.apply_clash_season_rollover(text,text,boolean) from public, anon;
grant execute on function private.apply_clash_season_rollover(text,text,boolean) to authenticated, service_role;

create function public.apply_clash_season_rollover(target_season_id text, target_player_id text default null, preview_only boolean default true)
returns jsonb language sql security invoker set search_path = '' as $$
  select private.apply_clash_season_rollover(target_season_id, target_player_id, preview_only);
$$;
revoke all on function public.apply_clash_season_rollover(text,text,boolean) from public, anon;
grant execute on function public.apply_clash_season_rollover(text,text,boolean) to authenticated, service_role;
