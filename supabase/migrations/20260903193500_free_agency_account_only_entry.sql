...
begin
  ...
  select application.id,
         ...
         coalesce(nullif(player.name, ''), nullif(profile.display_name, ''), 'Free Agent'),
         application.player_type,
         application.gender,
         coalesce(player.pdga_number, ''),
         player.pdga_rating,
         player.clash_index,
         coalesce(player.home_area, ''),
         application.created_at
  from public.launch_player_applications application
  join public.launch_profiles profile on profile.id = application.profile_id
  left join public.launch_players player on player.id = profile.player_id
  ...
  order by application.created_at asc,
           coalesce(nullif(player.name, ''), nullif(profile.display_name, ''), 'Free Agent') asc;
end;
$$;
